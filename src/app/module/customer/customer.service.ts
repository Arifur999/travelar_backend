 ...
 *
 * A cancelled hajj, tour or hotel booking contributes nothing, matching how
 * those modules report revenue everywhere else.
 *
...
const attachLedgerTotals = async <T extends { id: string; openingDue: Prisma.Decimal }>(
  ...
  const [ticketOwners, visaOwners, hajjOwners, tourOwners, hotelOwners] = await Promise.all([
  ...
  const hotelSale = new Map(
    hotelBookings.map((h) => [h.customerId, toNumber(h._sum.sellAmount)]),
  );
  ...
  const hotelPaid = foldByOwner(hotelOwners, hotelPayments, (r) => (r as { bookingId: string }).bookingId);
 ...
 * rules as attachLedgerTotals: every non-deleted ticket, visa case, Hajj,
 * tour and hotel booking, their payments, and due receipts. It used to list
...
const getCustomerLedger = async (agencyId: string, id: string) => {
  ...
  const [tickets, visaCases, hajjBookings, tourBookings, hotelBookings, dueReceipts] =
    await Promise.all([
    prisma.ticket.findMany({
    ...
    prisma.hotelBooking.findMany({
      where: { agencyId, customerId: id, isDeleted: false },
      select: {
        id: true, hotelName: true, city: true, guestName: true, sellAmount: true,
        status: true, checkIn: true, createdAt: true,
        payments: { include: { cashAccount: { select: { name: true } } } },
      },
    }),
  ...
  for (const booking of hotelBookings) {
    const label = `${booking.hotelName}, ${booking.city} — ${booking.guestName}`;
    const cancelled = booking.status === HotelBookingStatus.CANCELLED;

    // A cancelled stay bills nothing — the same rule the totals use — but it
    // stays on the statement so the payments taken against it still have a
    // line to belong to.
    events.push({
      date: booking.createdAt,
      type: "hotel",
      description: cancelled ? `Hotel — ${label} (cancelled, not billed)` : `Hotel — ${label}`,
      debit: cancelled ? 0 : toNumber(booking.sellAmount),
      credit: 0,
    });
    ...
    for (const payment of booking.payments) {
      events.push({
        date: payment.paidAt,
        type: "hotel-payment",
        description: paymentDescription(`Hotel payment — ${label}`, payment),
        debit: 0,
        credit: creditOf(payment),
      });
    }
