...
const recordPayment = async (
  ...
  // the customer is locked before the visa case (rowLock.ts). An edit could move
  // the visa case to another customer in between, so it is checked again below.
  const owner = fromWallet
    ? await prisma.visaCase.findFirst({
        where: { id: visaCaseId, agencyId, isDeleted: false },
        select: { customerId: true },
      })
    : null;
  if (fromWallet && !owner) throw new AppError(status.NOT_FOUND, "Visa case not found");

  await prisma.$transaction(async (tx) => {
    if (owner) await lockRow(tx, "customer", owner.customerId, agencyId);

    // So simultaneous payments for this case queue instead of each summing
    ...
    if (owner && visaCase.customerId !== owner.customerId) {
      throw new AppError(
        status.CONFLICT,
        "This case was moved to another customer while you were paying — try again",
      );
    }

    if (fromWallet) {
      await WalletService.assertCovers(tx, agencyId, visaCase.customerId, payload.amount);
    } else {
